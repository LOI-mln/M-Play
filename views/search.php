                                ...
                                <a href="/movies/details?id=<?= $movie['stream_id'] ?>&from=search&q=<?= urlencode($query) ?>"
                                    ...
                                    <div
                                        ...
                                        <h3 class="text-white font-bold text-sm truncate drop-shadow-md">
                                            <?= htmlspecialchars($movie['display_name']) ?></h3>
                ...
                <?php if (!empty($results['series'])): ?>
                    <section>
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <span class="px-2 py-0.5 rounded-full bg-purple-600 text-white text-xs font-bold">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                            <?php foreach ($results['series'] as $serie): ?>
                                <a href="/series/details?id=<?= $serie['series_id'] ?>&from=search&q=<?= urlencode($query) ?>"
                                    class="group relative block aspect-[2/3] bg-[#1a1a1a] rounded-xl overflow-hidden shadow-lg hover:shadow-purple-900/40 hover:scale-105 transition-all duration-300 ring-1 ring-white/10 hover:ring-purple-500">
                                    ...
                                    <div
                                        class="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/90 via-black/60 to-transparent">
                                        <h3 class="text-white font-bold text-sm truncate drop-shadow-md">
                                            <?= htmlspecialchars($serie['display_name']) ?></h3>
